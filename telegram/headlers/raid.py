...
# Генератор уникальных ID для рейдов
def generate_raid_id():
    return str(uuid.uuid4())[:8]

# ========== ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ БОЯ ==========
async def perform_raid_battle(raid, boss, callback=None):
    """Выполнить битву с боссом с учетом эффектов игроков"""
    
    total_damage = 0
    boss_hp = raid.get('boss_current_hp', boss.hp)
    players_names = []
    players_damage = []
    players_data = []  # Для хранения данных игроков
    
    # Очищаем истекшие эффекты у всех участников
    for user_id in raid['players']:
        player = None
        for p in api.get_all_players_global():
    ...
    for user_id in raid['players']:
        ...
        if player:
            ...
            # Получаем эффекты из чата игрока
...
# ========== КОМАНДА /raid - ПОКАЗАТЬ АКТИВНЫЕ РЕЙДЫ ==========
...
async def cmd_raid(message: types.Message):
    ...
    for raid_id, raid in active_raids.items():
        ...
        if boss:
            ...
            text += f"👥 {status}\n"
            text += f"❤️ HP: {raid.get('boss_current_hp', boss.hp)}\n"
...
async def cmd_join_by_name(message: types.Message):
    ...
    # Проверяем, не участвует ли игрок в другом рейде
    ...
    # Отправляем уведомление создателю рейда в его чат
    creator = None
    all_players = api.get_all_players_global()
    for p in all_players:
...
async def cmd_raid_status(message: types.Message):
    ...
    text += f"❤️ HP босса: {raid_data.get('boss_current_hp', boss.hp)}/{boss.hp}\n"
...
async def cmd_raid_battle(callback: types.CallbackQuery):
    ...
    await callback.answer("⚔️ Битва начинается!")
    
    # Уведомляем всех участников о начале битвы
    all_players = api.get_all_players_global()
    for user_id in raid['players']:
        try:
            for p in all_players:
    ...
    # Обновляем сообщение создателя
    await callback.message.edit_text("⚔️ <b>БИТВА НАЧАЛАСЬ!</b>\n\nРасчёт результатов...")
    ...
    # Формируем результат
    text = f"⚔️ <b>Битва с {boss.name}!</b>\n\n"
    ...
    # ПОБЕДА
        ...
        # Награждаем игроков с учетом повышения уровня
        level_up_messages = []
        for user_id in raid['players']:
            for p in all_players:
                if p.user_id == user_id:
    ...
    # Добавляем сообщения о повышении
        ...
        # ПОРАЖЕНИЕ
        text += f"\n👥 <b>ПОРАЖЕНИЕ!</b>\n"
        text += f"❤️ У босса осталось HP: {boss_hp}\n\n"
...
async def check_and_give_boss_achievements(boss_id, user_id, chat_id, bot):
    """Проверить и выдать достижение за убийство босса"""
